#!/usr/bin/env python3
"""
Moltbook Author-Submolt Interaction Network Analysis
=====================================================
Builds a co-posting network where authors are connected if they post in the same submolt.
"""
...
try:
    ...
    try:
        ...
        HAS_LOUVAIN = False
        print("Warning: python-louvain not installed. Will use networkx greedy_modularity_communities instead.")
...
def load_posts_data():
    ...
    FROM posts
    WHERE submolt_name != 'introductions'
    """

    df = pd.read_sql_query(query, conn)
    conn.close()

    print(f"Loaded {len(df)} posts (excluding 'introductions' submolt)")
    return df
...
def build_author_submolt_mapping(df):
    """Build mapping of author -> set of submolts they've posted in."""
...
def build_coposting_network(author_submolts, author_names):
    """
    Build network where two authors are connected if they posted in the same submolt.
...
def compute_network_stats(G, df, author_names):
    ...
    # Clustering coefficient (sample if too large)
    if G.number_of_nodes() > 5000:
        # Sample nodes for clustering
...
def run_community_detection(G, df):
    ...
    # Analyze dominant submolts in each community
    # Create author -> submolts mapping
...
def plot_degree_distribution(degrees, output_dir):
    ...
    axes[0].set_title('Degree Distribution (Histogram)', fontsize=14)
    ...
    axes[1].loglog(degrees_sorted, counts_sorted, 'o', markersize=4, alpha=0.7)
...
def plot_community_sizes(community_info, output_dir):
    ...
    bars = ax.bar(range(len(sizes)), sizes, color=plt.cm.tab20(np.linspace(0, 1, len(sizes))))
    ax.set_xticks(range(len(sizes)))
    ax.set_xticklabels(labels, rotation=45)
    ax.set_xlabel('Community', fontsize=12)
    ax.set_ylabel('Number of Authors', fontsize=12)
    ax.set_title('Community Sizes (Top 15)', fontsize=14)
...
def save_results(G, stats, partition, modularity, num_communities, community_info, output_dir):
    ...
    md_content = f"""# Moltbook Author Co-Posting Network Analysis
...
"""
    ...
    for comm in community_info:
        ...
        md_content += f"- Top submolts: {submolts_str}\n\n"
...
def main():
    print("=" * 60)
    print("MOLTBOOK AUTHOR CO-POSTING NETWORK ANALYSIS")
    print("=" * 60)
    print()
